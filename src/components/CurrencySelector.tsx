...
function useCurrencyList(...initialItems: (Currency|any)[]) {
  ...
  const list = useAsyncList<Currency, number>({
    ...
    initialSelectedKeys: initialItems.length ? initialItems.map(v => v.id) : undefined,
  });
  return list;
}
...
export default function CurrencySelector({
  ...
  const [selectedKey, setSelectedKey] = useState<number | string | undefined>();
  ...
  <>
    <Autocomplete
      ...
      inputProps={{
        classNames: {
          inputWrapper: 'h-full data-[hover=true]:border-blue-500 !border-default',
    ...
    >
      {(cur) => (
        <AutocompleteItem key={cur.id} value={cur.id} textValue={cur.symbol}>
          {cur.fullName}
        </AutocompleteItem>
      )}
    </Autocomplete>
